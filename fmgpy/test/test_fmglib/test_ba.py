...
def test_float():
    ...
    ba_float = fmgpy.fmglib.forest_calcs.ba(ba_2)
...
def test_ba_1_plot_3_trees():
    ...
    assert fmgpy.fmglib.forest_calcs.ba(ba_4) == 30


def test_ba_2_plots_1_tree():
    ba_7 = pd.DataFrame({"PLOT": [1, 2, 2],
                         "TR_SP": ["ACSA2", "NONE", ""]})
    # basal area = (tree count = 1 * baf = 10) / plot count = 2 = 5
    assert fmgpy.fmglib.forest_calcs.ba(ba_7) == 5


def test_ba_2_plots_2_trees():
    ba_5 = pd.DataFrame({"PLOT": [1, 2], "TR_SP": ["ACSA2", "ACSA2"]})
    # basal area = (tree count = 2 * baf = 10) / plot count = 2 = 10
    assert fmgpy.fmglib.forest_calcs.ba(ba_5) == 10
...
def test_ba_2_plots_3_trees():
    ...
    assert fmgpy.fmglib.forest_calcs.ba(ba_6) == 15
...
def test_ba_3_plots_3_trees():
    ...
    assert fmgpy.fmglib.forest_calcs.ba(ba_7) == 10
